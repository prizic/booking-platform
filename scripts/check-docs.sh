...
if grep -rInE "$patterns" --include='*.md' . ; then bad "secret-shaped literal above"; fi

note "4. Unvalidated compliance claims"
if grep -rInE '\b(is|are|fully|GDPR|PDPL|PCI|SAMA|ZATCA)[ -]*(compliant|compliance certified)\b' --include='*.md' . \
