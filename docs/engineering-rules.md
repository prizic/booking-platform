...
> **Status:** issues #3 and #4 provide the pnpm/Turborepo workspace, three application shells, local Supabase foundation, CI orchestration, and executable workspace, browser, release, and database-foundation gates. Issue #5 adds the semantic brand, localization, component, accessibility, RTL, reduced-motion, and visual matrices. Issue #6 adds tenant tables, safe API type generation, and the complete tenant-isolation/RLS matrix; booking concurrency remains owned by issue #11. See [local-setup.md](./local-setup.md) for the exact command and owner of every gate.
