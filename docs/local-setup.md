...
| Workspace Realtime broadcast | Covered by `pnpm test:db` where the Supabase realtime schema exists — issue #16. The broadcast is guarded, so a stack without that schema installs no trigger and no gate fails; a browser subscription is `N/A — not yet implemented, owned by issue #102`. |
| E2E | `pnpm test:e2e` | Identity/release smoke available — issue #4; the no-payment booking journey (EN/AR × mobile/desktop, validation, duplicate submission, lost slot, payment refusal) available — issue #12; the request-to-book outcome and the customer proposal link available — issue #13; the guest manage-booking link, its refusal state, and its step-up available — issue #14; guest cancellation and its lost-race recovery available — issue #15; all of them driven against the Client route handlers with platform DTO responses, so a browser-driven journey against a seeded live database remains `N/A — not yet implemented, owned by issue #94`; remaining journeys are `N/A — not yet implemented, owned by issues #16–#18` |
