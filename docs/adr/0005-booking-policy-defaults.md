...
| 5 | `service.assignment_mode` | enum | `any_available` | `any_available` \| `customer_choice` \| `round_robin` | service |
...
- **Assignment (5).** `any_available` picks the eligible staff/resource with the fewest confirmed bookings in the target local day, ties broken by lowest resource id — deterministic, so a retry picks the same one. `customer_choice` requires a selection and fails closed if the chosen resource is unavailable at write time. `round_robin` picks the eligible resource with the oldest `last_assigned_at`, updated inside the confirmation transaction.
...
- `round_robin` fairness is only as good as `last_assigned_at`, which cancellations distort; the fairness metric in ADR-0009 will show drift.
