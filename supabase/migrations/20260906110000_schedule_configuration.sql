...
create or replace function private.save_schedule_config_v1(
  p_tenant_id uuid, p_operation text, p_payload jsonb, p_expected_revision bigint
)
...
begin
  ...
  if p_operation='scope' then
    ...
    return query select v_scope_id,(select revision from app.schedule_scopes where tenant_id=p_tenant_id and id=v_scope_id);
    return;
  end if;
  ...
  if p_operation in ('time_off','holiday','blackout','maintenance','policy') then
    ...
    if p_operation='time_off' then
      ...
      return query select v_target,(select revision from app.time_off where tenant_id=p_tenant_id and id=v_target); return;
      ...
      return query select v_target,(select revision from app.holidays where tenant_id=p_tenant_id and id=v_target); return;
      ...
      if p_operation='blackout' then
        ...
        return query select v_target,(select revision from app.blackouts where tenant_id=p_tenant_id and id=v_target);
      else
        ...
        return query select v_target,(select revision from app.resource_maintenance_blocks where tenant_id=p_tenant_id and id=v_target);
      end if;
    else
      ...
      return query select v_target,(select revision from app.schedule_policy_overrides where tenant_id=p_tenant_id and id=v_target); return;
    end if;
  end if;
...
$$;
revoke execute on function private.save_schedule_config_v1(uuid,text,jsonb,bigint) from public;
grant execute on function private.save_schedule_config_v1(uuid,text,jsonb,bigint) to authenticated;
...
language sql security invoker set search_path='' as $$
  select * from private.save_schedule_config_v1(p_tenant_id,p_operation,p_payload,p_expected_revision);
$$;
revoke all on function api_v1.save_schedule_config_v1(uuid,text,jsonb,bigint) from public;
grant execute on function api_v1.save_schedule_config_v1(uuid,text,jsonb,bigint) to authenticated;

comment on function api_v1.save_schedule_config_v1(uuid,text,jsonb,bigint) is
...
language sql security invoker set search_path='' as $$
  select 'scope', s.id, s.id, s.location_id, s.staff_id, s.resource_id, null, null, null, null, null, null, s.time_zone, null, null, null, s.revision
